...
##Heavy Threonine.Norm.Area
pool.stats <- pool.dat %>%
     group_by(Compound.Name) %>%
     summarise(m = mean(`Heavy Threonine.Norm.Area`, na.rm=T), 
               sd = sd(`Heavy Threonine.Norm.Area`, na.rm=T), 
               rsd = sd/m)
rsd.dat <- rsd.dat %>% mutate(`Heavy Threonine.Norm.Area` =
                                   pool.stats$rsd)
```
