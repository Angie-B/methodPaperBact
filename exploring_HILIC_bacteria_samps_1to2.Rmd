...
```{r}
pool.dat <- dat %>% filter(type == "Poo") %>% filter(ID.duplicate == "Poo-0615")
pool.dat <- pool.dat[-(grep("overload",x=pool.dat$Notes)),]
...
ggplot(sub, aes(x=Compound.Name, y=rsd, 
     ...
     scale_shape_manual(values=c(19,19,19,19,15,15,15,15,17,17,17,17)) +
...
## Save out rsd data
```{r}
write.csv(long.rsd, file = "HILIC_1to2_RSD_NormalizationComparison_pool0615.csv")
```
